...
:::info
The VTOL functionality is achieved by switching/transitioning between two configurations stored in the FC.
VTOL specific configurations are Mixer Profiles with associated control profiles.
One profile set is for fixed-wing(FW) mode, one is for multi-copter (MC) mode.
Configuration/Settings other than Mixer/control profiles are shared among two modes.
:::
{/* ![Alt text](Screenshots/mixerprofile_flow.png) */}

1. _(Optional)_ Find a DIFF ALL file for your model and start from there if possible
