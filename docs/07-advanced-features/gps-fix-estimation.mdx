...
description: Estimate position when GPS has been lost
---
...
_Plane has to acquire GPS fix and store home position before takeoff. Estimation completely without GPS fix will not work_.
...
_It is important, that plane fly with specified speed in CRUISE mode. If you have set option "Increase cruise speed with throttle" - do not use it without GPS Fix._

_If pitot is available, pitot sensor data will be used instead of constant. It is not necessary to specify fw_reference_airspeed. However, it is still adviced to specify for the case of pitot failure._

_Note related command: to continue mission without RC signal, see command `set failsafe_mission_delay=-1`._
...
_GPS can be disabled only after: 1) initial GPS fix is acquired 2) in ARMED mode._
