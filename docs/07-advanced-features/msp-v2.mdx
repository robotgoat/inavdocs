...
description: Detailed information about the MSP V2 protocol.
---


## Overview

This page describes MSPV2 (MultiWii Serial Protocol Version 2).
MSP is the remote messaging protocol used by INAV and other flight controllers such as MultiWii, CleanFlight, and BetaFlight.

MSPV2 was introduced in INAV 1.73 for legacy commands, and is fully implemented (16bit commands) after 1.73 (i.e. 1.74 development branch and successors).
A MSP API version of 2 or greater indicates MSPV2 support.

## MSP Protocol

MSP is a request-response protocol.
MSP defines the side sending the request as "MSP Master" role and the side generating a response as "MSP Slave" role.

A specific device may function as both "MSP Master" and "MSP Slave" or implement only one role, depending on requirements.
Generally, a Groundstation software is an "MSP Master" and FC is an "MSP Slave", however in some use-cases a FC or any other device may be required to act as an "MSP Master" and "MSP Slave" concurrently.
...
* Limited message size. MSP v1 is limited to 255 byte message payloads.
This is already a problem and can only get worse.
The extant attempt to address this limitation in MSP v1 (the JUMBO frame extension) is a 'band-aid' and still suffers from the next restriction.
* Weak check-summing. MSP v1 uses a simple XOR checksum.
This is vulnerable to simple communications errors (transposed bits).
It can fail to detect common transmission corruptions.
...
* 16bit payload.
* crc8_dvb_s2 checksum algorithm.
This is a single byte CRC algorithm that is much more robust than the XOR checksum in MSP v1.
...
| 2 | type |  | '\<' / '>' / '!' see [Message Types](./msp-v2.mdx#message-types) |
| 3 | flag | ✔  | uint8, flag, See [Message Flags](./msp-v2.mdx#message-flags) |
...
It is possible to encapsulate V2 messages in a V1 message in a way that is transparent to the consumer.
This is implemented by setting the V1 function id to 255 and creating a payload of a V2 message without the first three header bytes.
...
![V2 in V1](/img/features-adv/msp-encapsulation.png)
...
| ----- | ---------- | ---- | ---- | ---- |
| 0x0000-0x00FE | Legacy     |  ✘   | INAV, MultiWii, BetaFlight, Cleanflight, BaseFlight    | [Legacy](http://www.multiwii.com/wiki/index.php?title=Multiwii_Serial_Protocol) and [INAV MSP Dev Docs](https://github.com/iNavFlight/inav/tree/master/docs/development/msp) |
| 0x1000-0x1EFF | Common messages   | ✔   | INAV    | [INAV MSP Dev Docs](https://github.com/iNavFlight/inav/tree/master/docs/development/msp) |
| 0x1F00-0x1FFF | Sensors connected via MSP  |  ✔   | INAV    |  [INAV MSP Dev Docs](https://github.com/iNavFlight/inav/tree/master/docs/development/msp)  |
| 0x2000-0x2FFF | INAV-specific     |  ✔   | INAV    | [INAV MSP Dev Docs](https://github.com/iNavFlight/inav/tree/master/docs/development/msp) |
...
New INAV messages should be defined as MSPv2.
This will avoid un-managed contention between Cleanflight derivative FCs in the legacy V1 space.
