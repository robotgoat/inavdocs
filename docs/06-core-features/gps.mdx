...
But ONLY do so according to the tables below.
Note how the maximum update rate can only be achieved with lower concurrent constellations.

The M9 can _theoretically_ run up to 25 Hz with all four constellations enabled.
...
**However** doing so can cause data to be lost randomly by the module during processing.
Which can reduce precision, making the benefit of running more concurrent constellations on some modules not as worthwhile.
...
But if it takes longer than 13 minutes, you likely have GNSS RF band interference coming from a hardware source in your model.
...
But keep in mind to change it if you change your location for holidays etc.
