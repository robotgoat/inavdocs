...
The link explains how `STICK_CENTER` works. While `STICK_CENTER_ONCE` is activated by the same method. But instead of disabling Airmode when the throttle is lowered below the `throttle_threshold`, it will keep Airmode active until you disarm. 
...
Also see [PIDFF Controller for Fixed Wings](https://github.com/iNavFlight/inav/wiki/Tune-INAV-PID%E2%80%90FF-controller-for-fixedwing) for additional information on this topic related to Fixedwings.
