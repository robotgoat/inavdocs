...
On multirotors, enabling _Surface Mode_ switches ALTHOLD to terrain-following mode, maintaining constant height above ground instead of absolute altitude.
...
The only _independent_ flight mode you should apply ALTHOLD with is ANGLE mode.
_However, ALTHOLD can be used by **advanced multicopter** users together with ACRO mode ONLY if the user understands the importance of maintaining smooth control over the copters attitude._
...
This should be tuned for your hardware by settings mentioned [earlier](./flight-modes-nav.mdx#using-althold-with-a-multicopter-mc).
...
**Please see other factors that will also effect the loiter radius** [below](./flight-modes-nav.mdx#fixed-wing-waypoint-tracking-accuracy-and-turn-smoothing)
